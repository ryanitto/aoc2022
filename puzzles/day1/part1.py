...
magical energy to deliver presents on Christmas. For that, their favorite snack is a special type of star fruit that
only grows deep in the jungle. The Elves have brought you on their annual expedition to the grove where the fruit
grows.
...
def run():
    calories_to_elves = [sum((int(s) for s in (e.split('\n')))) for e in lines.split('\n\n')]
    return max(calories_to_elves)
